...
ratios_data <- 
      read_tibble("C:/Users/user/Desktop/Aaron/R/Websites/invest-with-r/data/ratios_final.csv", date_format = "%m/%d/%Y") %>% 
      distinct() %>% 
      # Select variables that are ratios derived from the raw figures
      select(ticker, fundamentals_date, sector_yhoo, adj_return_6M, contains(c("ratio", "index", "scaled", "turnover", "_to_"))) %>% 
      # Remove variables with "decision" prefix as these are future-dated ratios
      select(-starts_with("decision"))
      
ratios_long <-
      ratios_data %>% 
      pivot_longer(-c(ticker, fundamentals_date, sector_yhoo), names_to = "field", values_to = "value")

# Number of tickers
ratios_long %>% distinct(ticker) %>% nrow()

# Number of fields
ratios_long %>% distinct(field) %>% nrow()
```
...
tickers_with_full_dates <-
      ...
      distinct() %>% 
      group_by(ticker) %>%
      filter(first(fundamentals_date) <= start_date & last(fundamentals_date) >= end_date) %>% 
      distinct(ticker) %>% 
      pull()
...
ratios_filtered <- 
      ...
      filter(between(fundamentals_date, start_date, end_date)) %>% 
      distinct()

# Get ratios of missing values
ratios_na <-
      ratios_filtered %>%
      select(-sector_yhoo, -fundamentals_date, -sector_yhoo) %>% 
...
```{r}
# View the ratio of missing data by ticker and field (for a sample of the data)
set.seed(123)
ratios_na %>% 
...
**sga_index**, **gross_margin_index**
and **depreciation_index** and others
have a significant amount of missing
...
```{r}
ratios_na %>% 
      # Choose the fields to use
      filter(field %in% fields_to_use) %>% 
      # Choose 10 random tickers
      filter(ticker %in% sample(tickers_to_use, 10)) %>%
      ggplot(aes(ticker, field, fill = na_ratio)) +
      geom_tile(color = "darkgray", lwd = 1) +
      # geom_text(aes(label = percent_format(accuracy = 1)(na_ratio)), color = "black", size = 4) +
      scale_fill_gradient(low = "white", high = "red", name = "", labels = percent) +
...
The data was filtered to include only tickers and fields with complete data for
the dates specified earlier.
...
**debt_ratio**, **lt_debt_ratio**,
**lt_debt_ratio_pct_chg_1Y**,
**days_sales_outstanding_index**,
**asset_quality_index**,
**sales_growth_index**,
**leverage_index**,
...
$debt\_ratio = \frac{total\,liabilities_t}{total\,assets_t}$

-   **debt_ratio** measures the
    percentage of assets that is
    financed by short-term and long-term
    debt.

$lt\_debt\_ratio = \frac{total\,liabilities_t - total\,current\,liabilities_t) }{total\_assets_t}$

-   **lt_debt_ratio** measures the
    proportion of assets that are
    financed with long-term debt. Debt
    is used for leverage and can
    benefits healthy companies to a
    certain extent. Excessively high
    long-term debt ratios can
...
$days\_sales\_outstanding\_index = \frac{days\,sales\,outsanding_t} {days\,sales\,outstanding_{t-12}}$
      where,
      $days\,sales\,outstanding = \frac{365} {(\frac{revenue_t} {net\,receivables})}$

-   **days_sales_outstanding_index**
    measures the ratio of the days sales outstanding ratio in the current year 
    to the that in the prior year

$asset\_quality\_index = \frac{(total\,assets_t - total\,current\,assets_t - property\,plant\,equipment_t)} {total\,assets_t}$

-   **asset_quality_index** is a measure
    of the percentage of total assets
    that is in the form of intangible
    assets. All else equal, relatively
    more intangible assets is
    undesirable.
...
ratios_filtered <-
      ratios %>% 
      left_join(ratios_data %>% select(ticker, fundamentals_date, sector_yhoo) %>% 
...
The response variable (return_sign) has about
...
There appears to be outlier data points
in the **sales_growth_index** and 
**lt_debt_ratio_pct_chg_1Y**
variables and possibly others. We will
try to remove these extreme values.
...
ratios_cleaned <- 
      ...
      mutate(value = ifelse(value < quantile(value, .005), median(value, na.rm = TRUE), value),
             value = ifelse(value > quantile(value, .995), median(value, na.rm = TRUE), value)) %>%
      pivot_wider(names_from = field, values_from = value)
...
model_recipe <- recipe(return_sign ~ current_ratio + scaled_total_accruals + 
                             scaled_net_operating_assets + asset_turnover +
                             free_cash_flow_to_assets,
                       data = data_juiced)
...
**scaled_net_operating_assets**,
**asset_turnover**, and
**free_cash_flow_to_assets** still have
