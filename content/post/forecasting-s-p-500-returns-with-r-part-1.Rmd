...
SP500_tbl %>%
      ...
      summarize(mean = mean(ROC.SP500),
                ...
                stdev = sd(ROC.SP500)) %>%
      mutate(agg_mean = mean(SP500_tbl$ROC.SP500),
             z = (mean - agg_mean) / stdev) %>%
      mutate_all(~ round(., 3)) %>%
      kable() %>%
...
The density plots seem to reveal that February, July, and November, especially, experience a high proportion of returns greater than zero. This can be visualized by noticing that the mean and medians of each of these months' returns are considerably greater than the zero-lines (in black) and their density curves are peaked on the right side of the zero-point. Let's look further. This would suggest that stocks held in these months experience consistently positive monthly returns.
...
# Conclusions
Based on the preceding analysis, it appears that stock returns cannot be effectively modeled with the handful of forecast models employed in this analysis. There appears to be too much randomness that cannot be effectively modeled.
