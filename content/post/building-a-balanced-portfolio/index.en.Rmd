...
perc_up_days %>% 
  ...
  select(`Asset Class` = ticker, `% Up Days` = perc_up) %>% 
  gt() %>% 
  cols_align(
    align = "center",
    columns = `% Up Days`
  ...
  tab_header(
    title = "Percent of Time the Price Went Up",
    subtitle = md("*How often did the price go up?*")
  )
```
...
port_performance %>% 
  ...
  mutate(percentile = percent_rank(-annual_stdev)) %>% 
  filter(portfolio %in% best_return) %>% 
  mutate(across(c(annual_stdev, percentile), ~round(.x, 2))) %>% 
  rename(Portolio = portfolio, `Ann. St. Dev.` = annual_stdev, Percentile = percentile) %>% 
  gt() #%>% 
  # tab_header(
  #   title = "Portfolio Volatility",
  #   subtitle = md("**Which portfolios had the lowest volatility?**")
  # )
