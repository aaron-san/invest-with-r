...
each asset's the monthly return.
...
including Treausury ETFs in the portfolio containing stocks
...
returns_xts <- readRDS("static/data/returns_xts.rds")
```
...
classes over the investment period.
...
????? Update ------ It turns out that even when adjusting for "risk," the top performing portfolios 
were also comprised almost entirely of U.S. stocks.

# Summary

Over the last `r time_period %/% 365` years and 
`r (time_period %% 365) %/% (365/12)` months it would have been generally 
better to invest almost all of one's cash into the overall U.S. stock market. 
