...
In machine learning a cost function is defined to measure the cumulative 
...
With linear models, the cost function is often defined as the squared sum of
errors, a quadratic function:

$sse = \sum^n_i \frac{1} {2} \times (y^{(i)} - h(x^{(i)}))^2$

where $$y^{(i)}$$ is ith observed value and $$h(x^{(i)})$$ is the ith predicted value
